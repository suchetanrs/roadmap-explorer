...
SensorSimulator::SensorSimulator(std::shared_ptr<nav2_util::LifecycleNode> node, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> explore_costmap_ros)
: node_(node)
{
...
void SensorSimulator::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
{
  ...
  const bool geometry_changed =
        ...
        std::fabs(explored_map_.info.resolution -
  ...
  if (!geometry_changed)
  {   // nothing changed → just keep the previous explored grid
      return;
  }
...
void SensorSimulator::updateAfterChangedGeometry(const nav_msgs::msg::OccupancyGrid::SharedPtr updated_msg)
{
...
void SensorSimulator::markRay(const geometry_msgs::msg::Pose & base_pose,
                              double ray_angle)
{
  std::lock_guard<std::recursive_mutex> lock(explored_map_mutex_);
